...
- installed runtime version: `1.9.0`
- apps repo dependency (pinned in `pyproject.toml`): `aindy-runtime>=1.9.0,<2.0`
- runtime `/api/version` recommendation: `>=1.0,<2.0`
- app-profile boot smoke on 1.9.0: `boot_profile=default-apps`, `app_plugins_loaded=True`, `app_plugin_count=17`

Floor raised to `1.9.0` to adopt v1.9.0 (additive/opt-in, no schema change): **FR-5** —
