...
| Nodus-native reasoning execution — hook ADOPTED (2026-07-17). The `register_nodus_workflow` hook shipped in aindy-runtime 1.7.0, and the app now registers a first-party native Nodus workflow at boot: `apps/analytics/nodus/reasoning_apply_v1.nd` via `apps/analytics/bootstrap.py::_register_nodus_workflows` (validated + compiled; verified registering at app-profile boot). **Remaining — BLOCKED on runtime (FR-5, diagnosed 2026-07-18).** Routing the reasoning `execution_intent` to `run_nodus_workflow(...)` was probed on aindy-runtime 1.8.0. Execute-to-completion now **works** (the `.nd` runs to a terminal state; `set_state` surfaces at `data.nodus_output_state`), resolving the old harness caveat. But the `.nd` cannot reach app reasoning from a public `run_nodus_workflow` call: `call_tool` is fail-closed on a scoped capability token that `run_nodus_workflow` doesn't thread (the param exists one layer down in `nodus_execution_service`), and `sys()` → kernel `dispatch_syscall` returns "Unknown syscall" for app-registered syscalls. No app-side path; handed off as **FR-5** in `docs/handoffs/RUNTIME_FEATURE_REQUESTS.md` (thread `granted_capabilities` through `run_nodus_workflow`, or resolve app syscalls in the VM). App adoption (flag-gated `AINDY_REASONING_NODUS_NATIVE`, behavior-neutral substrate swap) waits on that | analytics | M (runtime-gated) | when FR-5 ships |
