...
| Infinity support-system depth — explicit `UserFeedback` now nudges per-user KPI **weights** (Support System Step 5, 2026-06-29, `kpi_weight_service.adapt_kpi_weights`). Remaining: weight feedback into the KPI **score formulas** (not just weights); centralize the support-state snapshot (Step 1, structural); and consume observability + agent/async execution aggregates (Steps 3/4) — runtime-gated, need a runtime aggregate syscall/job before the app can adapt them. ARM `arm_metrics_service` KPI output is also still not consumed by Infinity (Infinity recomputes from raw analysis rows) | analytics | M | when deepening Infinity optimization |
